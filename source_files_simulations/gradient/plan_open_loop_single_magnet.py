"""
plan_open_loop_single_magnet.py

Sequential open-loop planner for the single-magnet scenario.

This follows the SAME workflow as the original plan_open_loop.py:
1. Query the path geometry at the current predicted arc length s.
2. Build the desired field direction from the local frame.
3. Solve the inverse field problem.
4. Use field magnitude to scale predicted forward progress.
5. Advance state and write one CSV row.

Only the scenario changed:
- from two magnets to one magnet
- same thresholds
- same phase stepping logic
- same speed scaling logic
"""
...
from planner_geometry_single_magnet import PathGeometry
from field_inverse_solver_single_magnet import FieldInverseSolver
...
OUTPUT_CSV = "commands_open_loop_single_magnet.csv"
        ...
        commands.append(
            {
                ...
                "mhat_x": float(m_hat[0]),
                "mhat_y": float(m_hat[1]),
                "mhat_z": float(m_hat[2]),
                "Bx_mT": float(B_vec_T[0] * 1e3),
                "By_mT": float(B_vec_T[1] * 1e3),
                "Bz_mT": float(B_vec_T[2] * 1e3),
...
if __name__ == "__main__":
    ...
    print("=== plan_open_loop_single_magnet ===")
